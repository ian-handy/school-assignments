...
alone, with G2 highest and G3 lowest (\Cref{fig:ratio}).

\begin{figure}[tb]
  \centering
  \includegraphics[width=\linewidth, alt={Boxplot of pseudocode-to-map TFD ratio by expertise group on the DFS Metal stimulus, showing G2 highest and G3 lowest, plotted on a log scale.}]{fig_ratio_inverted_u_dfs}
  \caption{%
    Pseudocode/Map TFD ratio per participant on the DFS DE Metal
    stimulus, log-scaled. G2 sits at the apex of the inverted-U; G3
    drops below G1 as advanced learners pull dwell off the
    pseudocode panel.
  }
  \label{fig:ratio}
\end{figure}
...
those found for TFD. \Cref{fig:tfd} shows the lead TFD scatter on
the DFS stimulus; \Cref{fig:summary} compresses the TFD and FC
results into a single side-by-side view; \Cref{fig:core} shows the
participant-level FC scatter combined across BFS and DFS for
reference.

\begin{figure*}[tb]
  \centering
  \includegraphics[width=\linewidth, alt={Scatter plot showing total fixation duration on pseudocode vs map for G1 G2 G3 on the DFS Metal stimulus, with regression lines showing G3's strongly negative slope and G1's flat slope.}]{fig_tfd_substitution_dfs}
  \caption{%
    Primary result, on the DFS DE Metal stimulus: per-participant
    TFD on pseudocode vs.\ TFD on map. G3 (advanced) shows a
    significant negative substitution ($\rho$=$-$.66, $p$=.002);
    G1 (novice) is flat. Lines are OLS fits per group.
  }
  \label{fig:tfd}
\end{figure*}
...
\begin{figure}[tb]
  \centering
  \includegraphics[width=\linewidth, alt={Lollipop chart of Spearman rho between pseudocode and map AOIs for TFD and fixation count, by expertise group, on the DFS Metal stimulus.}]{fig_correlation_summary_dfs}
  \caption{%
    DFS DE Metal: Spearman~$\rho$ between pseudocode and map for
    TFD (lead) and FC (secondary). The TFD column moves
    monotonically from $0$ to $-$.66; the FC column shows the
    directional sign reversal reported in the paper. Asterisk
    marks $p<.05$.
  }
  \label{fig:summary}
\end{figure}
...
\begin{figure*}[tb]
  \centering
  \includegraphics[width=\linewidth, alt={Two side-by-side scatter plots, one for BFS and one for DFS, both on the DE Metal stimulus, showing pseudocode TFD vs map TFD by expertise group with regression lines.}]{fig_bfs_vs_dfs_substitution}
  \caption{%
    Substitution split by algorithm reveals an interaction with
    stimulus difficulty. On BFS, every group substitutes strongly
    ($\rho$ ranges $-$.58 to $-$.79, all $p$<.05); the expertise
    gradient is compressed because even novices are already pulled
    into trade-off mode. On DFS, the gradient is monotonic from a
    flat G1 baseline ($\rho$=+.01) to a strongly negative G3
    ($\rho$=$-$.66, $p$=.002). The expertise effect of interest is
    therefore most clearly isolated by DFS.
  }
  \label{fig:bfsdfs}
\end{figure*}
...
$r$=$-$.67 in G3. \Cref{fig:coupling} shows the same effect
restricted to the DFS Metal corpus, where the gradient is
sharper (mean $|\rho|$ = .368, .483, .605 for G1, G2, G3). A
participant-level distributional view of every metric used in this
matrix is provided in \Cref{fig:battery}.

\begin{figure*}[tb]
  \centering
  \includegraphics[width=\linewidth, alt={Three heatmaps of absolute Spearman correlations between eight key gaze metrics for groups G1, G2, and G3 on the DFS Metal stimulus. The heatmaps grow visibly darker from G1 to G3.}]{fig_coupling_heatmap_dfs}
  \caption{%
    Pairwise $|\rho|$ between eight gaze metrics on the DFS DE
    Metal stimulus, per expertise group. Mean $|\rho|$ rises from
    .368 (G1) to .483 (G2) to .605 (G3); cells in the G3 panel are
    visibly darker, evidencing tighter inter-metric coupling at
    higher expertise.
  }
  \label{fig:coupling}
\end{figure*}
...
\begin{figure*}[tb]
  \centering
  \includegraphics[width=\linewidth, alt={Six violin plots showing TFD on pseudocode and map, ratio, scanner index, fixation depth, and switching rate by expertise group on the DFS Metal stimulus.}]{fig_metric_battery_dfs}
  \caption{%
    Participant-level distributions of the six metrics that drive
    the coupling matrix, on the DFS DE Metal stimulus. The TFD-on-map
    panel shows G3's elongated upper tail; the pseudocode/map ratio
    panel shows G3's downward shift; scanner index, fixation depth,
    and switching rate are stable across groups, indicating the
    substitution effect is not driven by a global pacing change.
  }
  \label{fig:battery}
\end{figure*}
