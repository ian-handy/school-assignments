...
\authorfooter{%
  \item Razieh Fathi is with [INSTITUTION?]. E-mail: [EMAIL?].
  \item Ian Handy is with [INSTITUTION?]. E-mail: [EMAIL?].
}
...
vs.\ DFS presentation are [COUNTERBALANCING ORDER?].

\subsection{Subjects}
\label{sec:subjects}

Sixty-two participants were recruited at [INSTITUTION?]; the
study was approved under IRB protocol [IRB NUMBER?]. Gaze was
...
according to [EXPERTISE MEASUREMENT METHOD?]:
\begin{itemize}
...
(\texttt{DE-dft.wmv}); length of each trial was [TRIAL LENGTH?].
...
All figure-caption statistics are reproducible end-to-end from the
exported Tobii \texttt{.xls} files through the full open
\texttt{regen\_figures.py} pipeline distributed within the
...
conducted at [STUDY LOCATION?] between October and November 2022.
After signing an informed consent form, each participant was seated
in front of the eye-tracking display and completed a
[CALIBRATION PROCEDURE?]. Then, participants viewed both their BFS
and DFS animations [VIEWING ORDER?]---each shown once, per
participant. Either as part of, or following this task, we also
[ADDITIONAL TASKS / CHECKS / QUESTIONNAIRES?]. Sessions took
approximately [SESSION DURATION?] minutes per participant.
[COMPENSATION?]. We recorded data using
[TOBII SOFTWARE + VERSION?] and
...
gaze. The same shape still holds when conditioning on the DFS
stimulus only, with G2 highest and G3 lowest
(\Cref{fig:results}(a)).
...
those found for TFD. We combine BFS and DFS trials per-participant
for all body-text statistics in this section, preserving the
original within-subject design. We also report the same analysis
restricted to the DFS subset of data (\Cref{fig:results}(b),
\Cref{fig:summary}); restricting to DFS keeps the direction of
substitution the same but pulls G3 from $\rho$=$-$.631 to
$\rho$=$-$.660 and leaves G1 close to zero ($\rho$=$+$.013).
The lead TFD scatter (in x-y space) is shown in
\Cref{fig:results}(b) and \Cref{fig:summary} compounds the TFD and
FC panels into one.
...
\begin{figure}[tb]
  ...
  \caption{%
    ...
    TFD (lead) and FC (secondary). The TFD column moves from $0$
    to $-$.66; the FC column shows the directional sign reversal
    noted in the paper. Asterisk marks $p<.05$.
  }
  \label{fig:summary}
\end{figure}
...
$r$=$-$.67 in G3. \Cref{fig:coupling} restricts the main effect to
the DFS Metal corpus, where the sharp gradient effect shows (mean
$|\rho|$ = .368, .483, .605 for G1, G2, G3). Scanner index,
fixation depth, and switching rate did not differ by group (all
$p$>.10, Kruskal-Wallis) ruling out a global pacing explanation.
...
\begin{figure*}[tb]
  ...
  \caption{%
    ...
    Metal stimulus. By group, per metric. Mean $|\rho|$ rises from
    .368 (G1) to .483 (G2) to .605 (G3); cells in the G3 panel grow
    darker, showing tighter inter-metric coupling at higher
    expertise.
  }
  \label{fig:coupling}
\end{figure*}
