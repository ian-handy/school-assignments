    ...
    public void loadCreatureSprites() {
        ...
        for (int i=0; i<4; i++) {
            ...
            for(int x=0; x<enemies;x++)
            	...
            	if(s.getArchType(x).compareTo("toughgrub")==0)
            		enemyAnim[x][i] = createGrubAnim(
            				images[i][imageIndex++], images[i][imageIndex++]);
            	else
            	if(s.getArchType(x).compareTo("speedyfly")==0)
            		enemyAnim[x][i] = createFlyAnim(
            				images[i][imageIndex++], images[i][imageIndex++], images[i][imageIndex++]);
            	else
            	if(s.getArchType(x).compareTo("jumpygrub")==0)
            		enemyAnim[x][i] = createGrubAnim(
            				images[i][imageIndex++], images[i][imageIndex++]);
            	else
            	if(s.getArchType(x).compareTo("flyingmonkey")==0)
            		enemyAnim[x][i] = createMonkeyAnim(
            				images[i][imageIndex++], images[i][imageIndex++], images[i][imageIndex++]);
            	else
            	if(s.getArchType(x).compareTo("wizardgrub")==0)
            		enemyAnim[x][i] = createGrubAnim(
            				images[i][imageIndex++], images[i][imageIndex++]);
        }
        ...
        for(int x=0; x<enemies;x++)
            ...
            if(s.getArchType(x).compareTo("toughgrub")==0)
            	enemySprites[x]=new ToughGrub(enemyAnim[x][0], enemyAnim[x][1],
            			enemyAnim[x][2], enemyAnim[x][3]);
            else
            if(s.getArchType(x).compareTo("speedyfly")==0)
            	enemySprites[x]=new SpeedyFly(enemyAnim[x][0], enemyAnim[x][1],
            			enemyAnim[x][2], enemyAnim[x][3]);
            else
            if(s.getArchType(x).compareTo("jumpygrub")==0)
            	enemySprites[x]=new JumpyGrub(enemyAnim[x][0], enemyAnim[x][1],
            			enemyAnim[x][2], enemyAnim[x][3]);
            else
            if(s.getArchType(x).compareTo("flyingmonkey")==0)
            	enemySprites[x]=new FlyingMonkey(enemyAnim[x][0], enemyAnim[x][1],
            			enemyAnim[x][2], enemyAnim[x][3]);
            else
            if(s.getArchType(x).compareTo("wizardgrub")==0)
            	enemySprites[x]=new WizardGrub(enemyAnim[x][0], enemyAnim[x][1],
            			enemyAnim[x][2], enemyAnim[x][3]);
    }
